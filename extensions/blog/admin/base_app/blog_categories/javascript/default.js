$(document).ready(function (){
	$('.insertIcon, .editIcon').click(function (){
		...
		js_redirect(js_app_link('appExt=blog&app=blog_categories&appPage=new_category&' + key + '=' + $thisRow.attr('data-category_id')));
	});
		...
		$('<div></div>').dialog({
			...
			buttons: {
				Confirm: function() {
					...
					$.ajax({
						cache: false,
						url: js_app_link('appExt=blog&app=blog_categories&appPage=default&action=deleteCategoryConfirm'),
						dataType: 'json',
						type: 'POST',
