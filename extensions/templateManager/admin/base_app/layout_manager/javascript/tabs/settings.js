	...
	$.extend($.ui.LayoutDesigner.prototype.tabs, {
		...
		settings: {
			...
			init: function () {
				...
				$Tab.find('select[name=is_anchor]')

					.val(values.is_anchor)

					.change(function () { thisCls.processInputs.apply(parentCls); });



				$Tab.find('select[name=anchor_id]')

					.val(values.anchor_id)
			...
			processInputs: function () {
				...
				this.updateInputVal('is_anchor', $Tab.find('select[name=is_anchor]').val());
				...
				this.updateInputVal('anchor_id', $Tab.find('select[name=anchor_id]').val());
