<?php
	...
	if ($banners_body = new upload('banners_body', sysConfig::getDirFsCatalog().'extensions/bannerManager/images/')) {
		if(tep_not_null($banners_body->filename))
			$Banner->banners_body = $banners_body->filename;
	}

	if ($banners_body_thumbs = new upload('banners_body_thumbs', sysConfig::getDirFsCatalog().'extensions/bannerManager/images/')) {
