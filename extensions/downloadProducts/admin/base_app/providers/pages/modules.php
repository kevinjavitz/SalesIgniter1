<?php
	...
	$tableGrid->addButtons(array(
		htmlBase::newElement('button')->setText('View Providers')->setHref(itw_app_link('appExt=downloadProducts', 'providers', 'default')),
	...
	$moduleDirs = array(
		sysConfig::getDirFsCatalog() . 'extensions/downloadProducts/providerModules/'
	);
	$extensions = $appExtension->getExtensions();
	foreach($extensions as $extCls){
		if ($extCls->isEnabled()){
			if (is_dir($extCls->getExtensionDir() . 'ext_app/downloadProducts/providerModules/')){
				$moduleDirs[] = $extCls->getExtensionDir() . 'ext_app/downloadProducts/providerModules/';
			}
		}
	}
	...
	foreach($moduleDirs as $dirName){
		...
		foreach($dirObj as $dir){
			...
			$className = 'DownloadProvider' . ucfirst($dir->getBasename());
...
<div class="pageHeading"><?php
	echo sysLanguage::get('HEADING_TITLE_PROVIDERS_MODULES');
?></div>
<br />
