$(document).ready(function (){
		...
		gridWindow({
			...
			onShow: function (){
				...
				$(self).find('.modulesBox').change(function (){
					...
					boxGetVars.push('appExt=downloadProducts');
						...
						success: function (data){
							if (data.success){
								if ($('.gridBodyRow.state-active').size() > 0){
									...
									js_redirect(js_app_link('app=providers&appExt=downloadProducts&appPage=default&pID=' + data.pID));
								}
							}else{
								alert(data.message);
							}
	...
	$('.viewButton').click(function (){
		...
		gridWindow({
			...
			onShow: function (){
				...
				$(self).find('li > ul').hide();
				
				$('.ui-icon-folder-collapsed').live('click', function (){
					$(this).parent().find('ul').first().show();
					$(this).removeClass('ui-icon-folder-collapsed').addClass('ui-icon-folder-open');
				});
				
				$('.ui-icon-folder-open').live('click', function (){
	...
	$('.deleteButton').click(function (){
		...
		confirmDialog({
			confirmUrl: js_app_link('app=providers&appExt=downloadProducts&appPage=default&action=deleteProvider&pID=' + providerId),
			...
			success: function (){
				js_redirect(js_app_link('app=providers&appExt=downloadProducts&appPage=default'));
			}
		});
	});
});