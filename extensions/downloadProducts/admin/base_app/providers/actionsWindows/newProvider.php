<?php
	$Providers = Doctrine_Core::getTable('ProductsDownloadProviders');
	if (isset($_GET['pID'])){
	...
	->select('modules_code')
	->from('Modules')
	->where('modules_type = ?', 'download_provider')
	->execute(array(), Doctrine_Core::HYDRATE_ARRAY);
	if ($Qmodules){
		foreach($Qmodules as $mInfo){
			require(sysConfig::getDirFsCatalog() . 'extensions/downloadProducts/providerModules/' . $mInfo['modules_code'] . '/module.php');
			$className = 'DownloadProvider' . ucfirst($mInfo['modules_code']);
