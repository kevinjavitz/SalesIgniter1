<?php
class DownloadProviderAmazons3 extends DownloadProviderModule {
	...
	public function __construct($config = false) {
		...
		$this->setDescription('Downloads are stored in Amazon S3');
	...
	public function getBucketName(){
		return $this->getConfigData('MODULE_DOWNLOAD_PROVIDER_AMAZONS3_BUCKET');
	}
	
	public function getBucketPath(){
		return $this->getConfigData('MODULE_DOWNLOAD_PROVIDER_AMAZONS3_BUCKET_PATH');
	}
	
	public function getKey(){
		return $this->getConfigData('MODULE_DOWNLOAD_PROVIDER_AMAZONS3_ACCESS_KEY');
	}
	
	public function getSecret(){
		return $this->getConfigData('MODULE_DOWNLOAD_PROVIDER_AMAZONS3_SECRET_KEY');
	}
	
	public function processDownload($dInfo){
		$protocol = $dInfo['download_type'];
		$filePath = $this->getBucketName() . $this->getBucketPath();
		...
		return array(
			'url' => $protocol . '://s3.amazonaws.com/' . $filePath . '?' . implode('&', $getVars)
		);
	}
