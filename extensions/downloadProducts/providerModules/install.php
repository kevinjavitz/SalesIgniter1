<?php
	class DownloadProviderInstaller {
		
		public function __construct($moduleCode, $extName = null){
			$this->moduleDir = sysConfig::getDirFsCatalog();
			if (is_null($extName) === false){
				$this->moduleDir .= 'extensions/' . $extName . '/providerModules/' . $moduleCode . '/';
			}else{
				$this->moduleDir .= 'extensions/downloadProducts/providerModules/' . $moduleCode . '/';
			}
			...
			$className = 'DownloadProvider' . ucfirst($moduleCode);
		...
		public function install(){
			if ($this->moduleCls->isInstalled() === false){
				...
				$moduleConfig->modules_type = 'download_provider';
