...
function setupWidgetInstaller() {
	$('.dropToInstall').droppable({
		...
		drop: function (e, ui) {
			...
			$.ajax({
				cache: false,
				url: js_app_link('rType=ajax&app=modules&appPage=infoboxes&action=pdfinstall&' + getVars.join('&')),
				dataType: 'json',
				beforeSend: function () {
	...
	$('.dropToUninstall').droppable({
		...
		drop: function (e, ui) {
			...
			$.ajax({
				cache: false,
				url: js_app_link('rType=ajax&app=modules&appPage=infoboxes&action=pdfremove&' + getVars.join('&')),
				dataType: 'json',
				beforeSend: function () {
					showAjaxLoader($this, 'xlarge');
				},
...
$(document).ready(function() {
	...
	$('.BrowseServerField').live('click focusout', function (e) {
		if (e.type == 'click'){
			browserField = $(this);
			currentFolder = 'extensions/pdfPrinter/images';
			window.open(
	...
	$('.translateText').live('click', function (){
		...
		$.ajax({
			cache: false,
			url: js_app_link('appExt=pdfPrinter&app=layout_manager&appPage=default&action=translateText&from=' + $(this).attr('data-from') + '&to=' + $(this).attr('data-to') + '&string=' + inputField.val()),
			dataType: 'html',
