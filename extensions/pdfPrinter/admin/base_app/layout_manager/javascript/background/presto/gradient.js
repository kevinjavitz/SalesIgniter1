(function($, undefined) {
	...
	$.fn.backgroundBuilder.presto.gradient = function (o) {
		...
		if (!inputVals.presto.background){
			inputVals.presto.background = {};
		}

		if (!inputVals.presto.background.gradient){
			inputVals.presto.background.gradient = {
				config       : {},
				colorStops   : [],
				imagesBefore : [],
				imagesAfter  : []
		...
		this.settings = '<span>Gradient Not Supported By Opera</span>';
		...
		this.showSettings = function () {
			var self = this;
			var inputVals = this.activeEl.data('styles_info').input_vals.presto.background.gradient;
			...
			$(this.sCont)
				.find('input[name=start_horizontal_pos], input[name=start_vertical_pos], input[name=end_horizontal_pos], input[name=end_vertical_pos]')
				.each(function () {
				var inputEl = this;
		...
		this.updatePreview = function () {
			...
			$(this.sCont).find('.gradientPreview').each(function () {
				$(this)
					.attr('style', 'border:1px solid #cccccc;height:100px;' + backgroundKey + ': ' + urlBeforeStr + gradientString + urlAfterStr);
			});
		};
		...
		this.updateGradient = function () {
			...
			inputVals.presto.background.gradient.config = {
			...
			inputVals.presto.background.gradient.colorStops = [];
			$sCont.find('.gradientStop').each(function () {
				inputVals.presto.background.gradient.colorStops.push({
			...
			inputVals.presto.background.gradient.imagesBefore = [];
			$sCont.find('.gradientImagesBefore .gradientImage').each(function () {
				inputVals.presto.background.gradient.imagesBefore.push({
			...
			inputVals.presto.background.gradient.imagesAfter = [];
			$sCont.find('.gradientImagesAfter .gradientImage').each(function () {
				inputVals.presto.background.gradient.imagesAfter.push({
					image_source     : $(this).find('input[name=image_source]').val(),
					image_attachment : $(this).find('select[name=image_attachment]').val(),
					image_pos_x      : $(this).find('input[name=image_pos_x]').val(),
					image_pos_y      : $(this).find('input[name=image_pos_y]').val(),
			...
			var StylesInfo = this.activeEl.data('styles_info');
			StylesInfo.input_vals.presto.background.gradient = inputVals.presto.background.gradient;
