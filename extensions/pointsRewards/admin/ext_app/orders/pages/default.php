<?php
	class pointsRewards_admin_orders_default extends Extension_pointsRewards {
		...
		public function load(){
			...
			foreach($Qorders as $order)	{
				...
				foreach($order['OrdersProducts'] as $product){
					...
					if(strstr($product['pointsRewardsPurchaseTypes'][0]['percentage'],'%')){
						$pointsEarned = $productsPrice * $product['pointsRewardsPurchaseTypes'][0]['percentage'] / 100 * $product['products_quantity'];
					}else{
						$pointsEarned = $product['pointsRewardsPurchaseTypes'][0]['percentage'] * $product['products_quantity'];
					}
