...
class multiStore_admin_bannerManager_banner_groups_new_group extends Extension_multiStore {
	...
	public function NewGroupTabBody(&$gInfo){
		...
		if (isset($gInfo['banner_group_id']) && $gInfo['banner_group_id'] > 0){
			...
			->from('BannerManagerGroupsToStores')
			->where('banner_group_id = ?', $gInfo['banner_group_id'])
