$(document).ready(function (){
	$('#removeWishlist').click(function(){
		$.ajax({
				cache: false,
				url: js_app_link('appExt=customerWishlist&app=account_addon&appPage=manage_wishlist&action=deleteWishlist'),
				dataType: 'json',
				type: 'POST',
	...
	$('#saveSettings').click(function(){
		$.ajax({
				cache: false,
				url: js_app_link('appExt=customerWishlist&app=account_addon&appPage=manage_wishlist&action=saveSettings'),
				dataType: 'json',
				type: 'POST',
				data: $(this).closest('form').serialize(),
				success: function (data){
	...
	$('#searchWishlist').click(function(){ $(this).closest('form').attr('action', js_app_link('appExt=customerWishlist&app=account_addon&appPage=manage_wishlist')).submit(); });

	$('#addCartWishlist').click(function(){
		var $self = $('.selectDialog');
		$.ajax({
				cache: false,
				url: js_app_link('appExt=customerWishlist&app=account_addon&appPage=manage_wishlist&action=addToCart'),
				dataType: 'json',
				type: 'POST',
				data: $(this).closest('form').serialize(),
				success: function (data){
					$self.html(data.calendar);
					$('.pprButttons').append(data.customerWishlistReservation);
					...
					$('.inCart').live('click', function(event) {
						...
						$.ajax({
							...
							url: js_app_link('appExt=customerWishlist&app=account_addon&appPage=manage_wishlist&action=saveReservation' ),
							success: function (data) {
								//update priceEx
					...
					$self.css('z-index', '999');
				 	var posi = $('form[name=manage_wishlist]').offset();
