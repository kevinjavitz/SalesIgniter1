function showOptionEntry(el){
	if (el.value == 'select' || el.value == 'radioGroup' || el.value == 'checkboxGroup' || el.value == 'select' || el.value == 'selectOther'){
        $('#selectOptions').show();
	}else{
        $('#selectOptions').hide();
	}
	...
	$el.each(function (){
		...
		$(this).droppable({
			...
			drop: function (e, ui){
				...
				$.ajax({
					cache: false,
					url: js_app_link('appExt=customerCustomFields&app=manage&appPage=default&action=addFieldToGroup&group_id=' + $this.attr('group_id') + '&field_id=' + $('.fieldName', ui.draggable).attr('field_id')),
					dataType: 'json',
					beforeSend: function (){
	...
	$el.each(function (){
		$(this).droppable({
			...
			drop: function (e, ui){
				...
				$(ui.draggable).remove();
				$.ajax({
					url: js_app_link('appExt=customerCustomFields&app=manage&appPage=default&action=removeFieldFromGroup&group_id=' + $(this).parent().attr('group_id') + '&field_id=' + $(ui.draggable).attr('id')),
					cache: false,
					beforeSend: function (){
...
function makeListSortable($el){
	$el.each(function (){
		...
		$(this).sortable({
			...
			stop: function (e, ui){
				$.ajax({
					url: js_app_link('appExt=customerCustomFields&app=manage&appPage=default&action=updateFieldSortOrder&group_id=' + $(this).parent().attr('group_id') + '&' + $(this).sortable('serialize')),
					beforeSend: function (){
						showAjaxLoader($this.parent(), 'xlarge');
					},
					complete: function (){
			...
			$('<div></div>').dialog({
				...
				buttons: {
					'Save': function (){
						...
						$.ajax({
							cache: false,
							url: js_app_link('appExt=customerCustomFields&app=manage&appPage=default&action=saveField&fID=' + fieldId),
			...
			$('<div></div>').dialog({
				...
				buttons: {
					'Save': function (){
						...
						$.ajax({
							cache: false,
							url: js_app_link('appExt=customerCustomFields&app=manage&appPage=default&action=saveGroup&gID=' + groupId),
							data: $('.ui-dialog-content *', self.element).serialize(),
							type: 'post',
							dataType: 'json',
	...
	$('#newField').click(function (){
		$('<div></div>').dialog({
			...
			open: function (e, ui){
				...
				$.ajax({
					cache: false,
					url: js_app_link('appExt=customerCustomFields&app=manage&appPage=default&windowAction=new&action=getFieldWindow'),
					dataType: 'html',
					success: function (data){
						$(self).html(data);
					}
			...
			buttons: {
				'Save': function (){
					...
					$.ajax({
						cache: false,
						url: js_app_link('appExt=customerCustomFields&app=manage&appPage=default&action=saveField'),
						data: $('.ui-dialog-content *', self.element).serialize(),
						type: 'post',
						dataType: 'html',
						success: function (data){
	...
	$('#newGroup').click(function (){
		$('<div></div>').dialog({
			...
			open: function (e, ui){
				...
				$.ajax({
					cache: false,
					url: js_app_link('appExt=customerCustomFields&app=manage&appPage=default&windowAction=new&action=getGroupWindow'),
					dataType: 'json',
					success: function (data){
						$(self).html(data.html);
					}
			...
			buttons: {
				'Save': function (){
					...
					$.ajax({
						cache: false,
						url: js_app_link('appExt=customerCustomFields&app=manage&appPage=default&action=saveGroup'),
						data: $('.ui-dialog-content *', self.element).serialize(),
						type: 'post',
						dataType: 'html',
						success: function (data){
