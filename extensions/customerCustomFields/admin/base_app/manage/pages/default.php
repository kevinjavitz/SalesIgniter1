<?php
	...
	if ($Qfields->count() > 0){
		...
		foreach($Qfields->toArray(true) as $fInfo){
			...
			$fieldName = $fInfo['CustomerCustomFieldsDescription'][Session::get('languages_id')]['field_name'];
			$inputType = $fInfo['input_type'];

			$deleteIcon = htmlBase::newElement('icon')->setType('circleClose')->setTooltip('Click to delete field')
			->setHref(itw_app_link('appExt=customerCustomFields&action=removeField&field_id=' . $fieldId))
			->css($iconCss);

			$editIcon = htmlBase::newElement('icon')->setType('wrench')->setTooltip('Click to edit field')
			->setHref(itw_app_link('appExt=customerCustomFields&windowAction=edit&action=getFieldWindow&fID=' . $fieldId))
			->css($iconCss);

			$htmlFields = '<b><span class="fieldName" field_id="' . $fieldId . '">' . $fieldName . '</span></b>' . $deleteIcon->draw() . $editIcon->draw() . '<br />' . sysLanguage::get('TEXT_TYPE') . '<span class="fieldType">' . $inputType . '</span><br />';

			EventManager::notify('CustomerCustomFieldsAddOptions', &$htmlFields, $fInfo);
	...
	->from('CustomerCustomFieldsGroups')
	->orderBy('group_name')
	...
	if ($Qgroups){
		...
		foreach($Qgroups as $gInfo){
			...
			->from('CustomerCustomFields f')
			->leftJoin('f.CustomerCustomFieldsDescription fd')
			->leftJoin('f.CustomerCustomFieldsToGroups f2g')
			->where('fd.language_id = ?', Session::get('languages_id'))
			...
			if ($Qfields->count() > 0){
				foreach($Qfields->toArray(true) as $fInfo){
					...
					->attr('sort_order', $fInfo['CustomerCustomFieldsToGroups'][0]['sort_order'])
					->html($fInfo['CustomerCustomFieldsDescription'][Session::get('languages_id')]['field_name']);
					$sortableList->addItemObj($liObj);
				}
			}

			$deleteIcon = htmlBase::newElement('icon')->setType('circleClose')->setTooltip('Click to delete group')
			->setHref(itw_app_link('appExt=customerCustomFields&action=removeGroup&group_id=' . $groupId))
			->css($iconCss);

			$editIcon = htmlBase::newElement('icon')->setType('wrench')->setTooltip('Click to edit group')
			->setHref(itw_app_link('appExt=customerCustomFields&action=getGroupWindow&group_id=' . $groupId))
			->css($iconCss);
