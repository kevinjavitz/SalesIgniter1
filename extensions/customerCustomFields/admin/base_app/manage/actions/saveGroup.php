<?php
	$CustomerCustomFieldsGroups = Doctrine_Core::getTable('CustomerCustomFieldsGroups');
	if (isset($_GET['gID'])){
		$Group = $CustomerCustomFieldsGroups->findOneByGroupId((int)$_GET['gID']);
	}else{
		$Group = $CustomerCustomFieldsGroups->create();
	}
	...
	if (isset($_GET['gID'])){
		...
		$iconCss = array(
 		...
 		->setHref(itw_app_link('appExt=customerCustomFields&action=removeGroup&group_id=' . $Group->group_id))
 		->css($iconCss);

 		$editIcon = htmlBase::newElement('icon')->setType('wrench')->setTooltip('Click to edit group')
 		->setHref(itw_app_link('appExt=customerCustomFields&action=getGroupWindow&group_id=' . $Group->group_id))
 		->css($iconCss);
