	 ...
	 function DownloadProducts()
	{
		...
		if ($GLOBALS['ISC_CLASS_DB']->FetchOne($CountQuery) > $start) {
			...
			while ($product = $GLOBALS['ISC_CLASS_DB']->Fetch($products)) {
				// @todo Update this to work with changes to the product images in ISC-102
				$fullImage = $GLOBALS['ShopPath'] . '/product_images/' . $product['imagefile'];
				$fullImage = str_replace("_thumb","",$fullImage);
