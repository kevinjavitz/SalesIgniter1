<?php
	...
	$FormTable->addRow(tep_draw_input_field('email_address', $QAccount[0]['customers_email_address'],'readonly="readonly"') . '<a style="display: inline-block;" tooltip="Input Required" class="ui-icon ui-icon-gear ui-icon-required"></a>');
	if($QAccount[0]['customers_gender'] == 'm'){
		$checkedm = true;
		$checkedf = false;
	}else{
		$checkedm = false;
	...
	$FormTable->addRow(tep_draw_radio_field('gender', 'm',$checkedm) . '&nbsp;&nbsp;' . sysLanguage::get('MALE') . '&nbsp;&nbsp;' . tep_draw_radio_field('gender', 'f', $checkedf) . '&nbsp;&nbsp;' . sysLanguage::get('FEMALE') . '&nbsp;' );


	$FormTable->addRow(sysLanguage::get('ENTRY_DATE_OF_BIRTH'));
	$FormTable->addRow(tep_draw_input_field('dob', $QAccount[0]['customers_dob']) . '&nbsp;');

	$FormTable->addRow(sysLanguage::get('ENTRY_FIRST_NAME'), sysLanguage::get('ENTRY_LAST_NAME'));
	$FormTable->addRow(tep_draw_input_field('firstname',$QAccount[0]['customers_firstname']) . '&nbsp;' . '<a style="display: inline-block;" tooltip="Input Required" class="ui-icon ui-icon-gear ui-icon-required"></a>', tep_draw_input_field('lastname',$QAccount[0]['customers_lastname']) . '&nbsp;' . '<a style="display: inline-block;" tooltip="Input Required" class="ui-icon ui-icon-gear ui-icon-required"></a>');
...
<?php
	...
	$pageContent->set('pageForm', array(
		'name' => 'create_account',
		'action' => itw_app_link('action=saveAccount&appExt=subAccounts&cID='.$cID, 'manage', 'create', 'SSL'),
		'method' => 'post'
	));
