...
class metaTags_admin_articleManager_articles_new extends Extension_metaTags {
	...
	public function load(){
		...
		EventManager::attachEvents(array(
			...
			'ArticleManagerDescriptionsBeforeSave'
		), null, $this);
	}
	...
	/**
	 * listen for ArticleManagerFormMiddle event (fired by core)
	 ...
	 * @return string
	 */
	public function ArticleManagerFormMiddle($langid, &$content) {
		...
		if (isset($_POST['metatags'])) {
			...
			global $Article;
			if (isset($Article->ArticlesDescription[$langid])) {
				$values = array(
					't' => $Article->ArticlesDescription[$langid]->articles_head_title_tag,
					'd' => $Article->ArticlesDescription[$langid]->articles_head_desc_tag,
					'k' => $Article->ArticlesDescription[$langid]->articles_head_keywords_tag
				);
			}
		}
	...
	/**
	 * listen for ArticleManagerDescriptionsBeforeSave event (fired by core)
	 ...
	 * @return string
	 */
	public function ArticleManagerDescriptionsBeforeSave($descriptions) {

		if (isset($_POST['metatags'])) {
			foreach ($_POST['metatags'] as $langid => $vals) {
				$descriptions[$langid]->articles_head_title_tag		= $vals['t'];
				$descriptions[$langid]->articles_head_desc_tag 		= $vals['d'];
				$descriptions[$langid]->articles_head_keywords_tag	= $vals['k'];
			}
		}
