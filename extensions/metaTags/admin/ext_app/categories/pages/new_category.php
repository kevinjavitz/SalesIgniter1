...
class metaTags_admin_categories_new_category extends Extension_metaTags {
	...
	public function load(){
		...
		EventManager::attachEvents(array(
			...
			'CategoriesDescriptionsBeforeSave'
		), null, $this);
	}
	...
	/**
	 * listen for CategoriesFormMiddle event (fired by core)
	 ...
	 * @return string
	 */
	public function CategoriesFormMiddle($langid, &$content) {
		...
		if (isset($_POST['metatags'])) {
			...
			global $Category;
			if (isset($Category->CategoriesDescription[$langid])) {
				$values = array(
					't' => $Category->CategoriesDescription[$langid]->categories_htc_title_tag,
					'd' => $Category->CategoriesDescription[$langid]->categories_htc_desc_tag,
					'k' => $Category->CategoriesDescription[$langid]->categories_htc_keywords_tag
				);
			}
		}
	...
	/**
	 * listen for CategoriesDescriptionsBeforeSave event (fired by core)
	 ...
	 * @return string
	 */
	public function CategoriesDescriptionsBeforeSave($descriptions) {

		if (isset($_POST['metatags'])) {
			foreach ($_POST['metatags'] as $langid => $vals) {
				$descriptions[$langid]->categories_htc_title_tag		= $vals['t'];
				$descriptions[$langid]->categories_htc_desc_tag 		= $vals['d'];
				$descriptions[$langid]->categories_htc_keywords_tag		= $vals['k'];
			}
		}
	}
