<?php
	class postaffiliatepro_catalog_account_default extends Extension_postaffiliatepro {
		...
		public function AccountDefaultAddLinksBlock(&$pageContents){
			...
			if(!$error){
			$session1 = new Gpf_Api_Session(sysConfig::get('HTTP_SERVER').'/affiliate/scripts/server.php');
			if(!$session1->login($affiliate->getUsername(), $affiliate->getPassword(), Gpf_Api_Session::AFFILIATE)) {
				...
				->setHref($session1->getUrlWithSessionInfo(sysConfig::get('HTTP_SERVER').'/affiliate/affiliates/panel.php'));
				...
				$link = sysConfig::get('HTTP_SERVER').'/gse/'.$QUsernameIds->username;
