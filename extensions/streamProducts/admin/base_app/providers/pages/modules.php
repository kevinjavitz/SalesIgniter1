...
$tableGrid->addButtons(array(
	htmlBase::newElement('button')->setText('View Providers')->setHref(itw_app_link('appExt=streamProducts', 'providers', 'default')),
...
$moduleDirs = array(
	sysConfig::getDirFsCatalog() . 'extensions/streamProducts/providerModules/'
);
$extensions = $appExtension->getExtensions();
foreach ($extensions as $extCls){
	if ($extCls->isEnabled()){
		if (is_dir($extCls->getExtensionDir() . 'ext_app/streamProducts/providerModules/')){
			$moduleDirs[] = $extCls->getExtensionDir() . 'ext_app/streamProducts/providerModules/';
		}
	}
}
...
foreach ($moduleDirs as $dirName){
	...
	foreach ($dirObj as $dir){
		...
		$className = 'StreamProvider' . ucfirst($dir->getBasename());
