...
if ($Qhistory){
	...
	$ListingTable->addHeaderRow(array(
		...
		'columns' => array(
			...
			array(
				'align' => 'left',
				'css'   => array('color' => '#FFFFFF'),
				'text'  => sysLanguage::get('TABLE_HEADING_FILENAME')
			),
			array(
				'align' => 'left',
				'css'   => array('color' => '#FFFFFF'),
			...
			array(
				'align' => 'left',
				'css'   => array('color' => '#FFFFFF'),
				'text'  => sysLanguage::get('TABLE_HEADING_DATE_EXPIRES')
			),
			array(
				'css'  => array('color' => '#FFFFFF'),
				'text' => sysLanguage::get('TABLE_HEADING_ACTION')
			),
	...
	foreach ($Qhistory as $oInfo){
		foreach ($oInfo['OrdersProducts'] as $opInfo){
			...
			if (!isset($opInfo['OrdersProductsStream']) || empty($opInfo['OrdersProductsStream'])) {
				continue;
			}
			...
			foreach ($opInfo['OrdersProductsStream'] as $opsInfo){
				...
				$ListingTable->addBodyRow(array(
					'columns' => array(
						...
						array(
							'align' => 'center',
							'text'  => $streamButton . '<br>' . $orderButton
						)
					)
				));
			}
