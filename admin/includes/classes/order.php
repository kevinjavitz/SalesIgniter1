...
class order {
	...
	function query($order_id) {
		...
		while ($orders_products = tep_db_fetch_array($orders_products_query)) {
			...
			$Qcheck = tep_db_query('select * from rental_bookings where orders_products_id = "' . $orders_products['orders_products_id'] . '" and parent_id is null');
			if (tep_db_num_rows($Qcheck)){
				$check = tep_db_fetch_array($Qcheck);
				$this->products[$index]['reservation'] = array(
					'barcode'    => $check['products_barcode'],
					'start_date' => $check['start_date'],
					'end_date'   => $check['end_date'],
					'insure'     => $check['insured'],
					'insure_per' => $check['insured_per'],
					'quantity'   => $check['rental_qty']
				);

				if (!empty($check['shipping_method'])){
					include_once('../includes/modules/shipping/reservation.php');
					$module = new reservation();
					$quote = $module->quote($check['shipping_method']);
					
					$this->products[$index]['reservation']['shipping'] = array(
						'title' => $quote['methods'][0]['title'],
						'cost'  => $quote['methods'][0]['cost'],
						'id'    => $quote['methods'][0]['id'],
						'days'  => $quote['methods'][0]['days']
					);
				}
			}

			$QdownloadCheck = tep_db_query('select count(*) as total from orders_products_download where orders_products_id = "' . $orders_products['orders_products_id'] . '"');
			$downloadCheck = tep_db_fetch_array($QdownloadCheck);
			if ($downloadCheck['total'] > 0){
				$this->products[$index]['download_type'] = 'download';
			}

			$QstreamCheck = tep_db_query('select count(*) as total from orders_products_stream where orders_products_id = "' . $orders_products['orders_products_id'] . '"');
			$streamCheck = tep_db_fetch_array($QstreamCheck);
			if ($streamCheck['total'] > 0){
...
function getProductsName($productsID){
	foreach($this->products as $pInfo){
		if ($pInfo['id'] == $productsID){
			return $pInfo['name'];
			break;
		}
	}
