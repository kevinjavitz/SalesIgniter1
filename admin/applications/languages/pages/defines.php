		...
		foreach($Regex as $arr){
			...
			if ($skipFile === false){
				...
				for($i=0, $n=sizeof($path); $i<$n; $i++){
					...
					if ($path[$i] == 'language_defines') continue;
...
<?php
	...
	$fromLangDrop->selectOptionByValue((isset($_GET['langCode']) ? $_GET['langCode'] : 'auto'));
			...
			<tr>
				<td><table cellpadding="3" cellspacing="0" border="0" width="100%">
					<tr>
						...
						<td width="125"><?php echo htmlBase::newElement('button')->setId('searchButton')->setText('Search Files')->draw();?></td>
					</tr>
					<tr>
