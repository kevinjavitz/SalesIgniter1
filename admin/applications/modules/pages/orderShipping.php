<?php
	...
	$moduleDirs = array(
		sysConfig::getDirFsCatalog() . 'includes/modules/orderShippingModules/'
	);
	$extensions = $appExtension->getExtensions();
	foreach($extensions as $extCls){
		if ($extCls->isEnabled()){
			if (is_dir($extCls->getExtensionDir() . 'orderShippingModules/')){
				$moduleDirs[] = $extCls->getExtensionDir() . 'orderShippingModules/';
			}
		}
	}
	...
	foreach($moduleDirs as $dirName){
		...
		foreach($dirObj as $dir){
			...
			$className = 'OrderShipping' . ucfirst($dir->getBasename());
	...
	foreach($modules as $moduleCode => $moduleCls){
		...
		$tableGrid->addBodyRow(array(
			'rowAttr' => array(
				...
				'data-module_type' => 'orderShipping',
			...
			'columns' => array(
				...
				array('text' => $installedIcon->draw(), 'align' => 'center'),
...
<div class="pageHeading"><?php
	echo sysLanguage::get('HEADING_TITLE_MODULES_SHIPPING');
?></div>
<br />
