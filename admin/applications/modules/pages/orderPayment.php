<?php
	...
	$moduleDirs = array(
		sysConfig::getDirFsCatalog() . 'includes/modules/orderPaymentModules/'
	);
	$extensions = $appExtension->getExtensions();
	foreach($extensions as $extCls){
		if ($extCls->isEnabled()){
			if (is_dir($extCls->getExtensionDir() . 'orderPaymentModules/')){
				$moduleDirs[] = $extCls->getExtensionDir() . 'orderPaymentModules/';
			}
		}
	}
	...
	foreach($moduleDirs as $dirName){
		...
		foreach($dirObj as $dir){
			...
			$className = 'OrderPayment' . ucfirst($dir->getBasename());
...
<div class="pageHeading"><?php
	echo sysLanguage::get('HEADING_TITLE_MODULES_PAYMENT');
?></div>
<br />
