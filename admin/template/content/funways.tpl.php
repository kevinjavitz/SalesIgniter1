...
<?php
    echo tep_draw_form('search', FILENAME_FUNWAYS, '', 'get');
...
?>
              ...
              <tr>
                <td class="smallText" align="right">
<?php
    echo tep_draw_form('goto', FILENAME_FUNWAYS, '', 'get');
...
?>
      ...
      <tr>
        <td>
<?php
    echo tep_draw_form('select', FILENAME_FUNWAYS, '', 'get');
?>
...
// Get parent_id for subcategories if search
    ...
    //  $prods_count = tep_count_funways_products_in_category($categories['categories_id']);
      if (isset($cInfo) && is_object($cInfo) && ($categories['categories_id'] == $cInfo->categories_id) ) {
        ...
        $click = tep_href_link(FILENAME_FUNWAYS, tep_get_path($categories['categories_id']));
      } else {
        echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)">' . "\n";
        $click = tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&cID=' . $categories['categories_id']);
      }
?>
                ...
                <td class="dataTableContent" onclick="document.location.href='<? echo $click;?>'"><?php echo '<a href="' . tep_href_link(FILENAME_FUNWAYS, tep_get_path($categories['categories_id'])) . '">' . tep_image(DIR_WS_ICONS . 'folder.gif', ICON_FOLDER) . '</a>&nbsp;<b>' . $categories['categories_name'] . '</b>'; ?></td>
                ...
                <td class="dataTableContent" onclick="document.location.href='<? echo $click;?>'" align="right"><?php if (isset($cInfo) && is_object($cInfo) && ($categories['categories_id'] == $cInfo->categories_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&cID=' . $categories['categories_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
...
// find out the rating average from customer reviews
      ...
      if (isset($pInfo) && is_object($pInfo) && ($products['products_id'] == $pInfo->products_id) ) {
        echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)">' . "\n";
        $click = tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&pID=' . $products['products_id']);
      } else {
        echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)">' . "\n";
        $click = tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&pID=' . $products['products_id']);
      }
...
?>
                ...
                <td class="dataTableContent" onclick="document.location.href='<?echo $click; ?>'"><?php echo '<a href="' . tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&pID=' . $products['products_id'] . '&action=new_product_preview&read=only') . '">' . tep_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW) . '</a>&nbsp;' . $funways_product['products_name'];?></td>
                <td class="dataTableContent" align="center" onclick="document.location.href='<?echo $click; ?>'" align="center">Product</td>
                <td class="dataTableContent" align="center" onclick="document.location.href='<?echo $click; ?>'"><a href="<? echo tep_href_link(FILENAME_CATEGORIES, 'pID=' . $funways_product['products_id']).'&action=new_product'?>">[Edit]</a></td>
                <td class="dataTableContent" onclick="document.location.href='<?echo $click; ?>'" align="center">Product</td>
                <td class="dataTableContent" align="right"><?php if (isset($pInfo) && is_object($pInfo) && ($products['products_id'] == $pInfo->products_id)) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&pID=' . $products['products_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              	</tr>
<?php
    }
                ...
                <td colspan="6"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td align="right" class="smallText"><?php
                     if (sizeof($cPath_array) > 0) echo htmlBase::newElement('button')->usePreset('back')->setHref(tep_href_link(FILENAME_FUNWAYS, $cPath_back . 'cID=' . $current_category_id))->draw() . '&nbsp;'; if (!isset($_GET['search'])) echo htmlBase::newElement('button')->setText(sysLanguage::get('IMAGE_NEW_CATEGORY'))->setHref(tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&action=new_category'))->draw() . '&nbsp;' . htmlBase::newElement('button')->setText(IMAGE_NEW_PRODUCT)->setHref(tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&action=new_product'))->draw(); ?>&nbsp;</td>
                  </tr>
                </table></td>
              </tr>
            </table>
...
<?php
    ...
    switch ($action) {
      case 'new_category':
        ...
        $contents = array('form' => tep_draw_form('newcategory', FILENAME_FUNWAYS, 'action=insert_category&cPath=' . $cPath, 'post', 'enctype="multipart/form-data"'));
        ...
        $contents[] = array('align' => 'center', 'text' => '<br>' . htmlBase::newElement('button')->usePreset('save')->setType('submit')->draw() . ' ' . htmlBase::newElement('button')->usePreset('cancel')->setHref(tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath))->draw());
        break;
      case 'edit_category':
        $heading[] = array('text' => '<b>' . sysLanguage::get('TEXT_INFO_HEADING_EDIT_CATEGORY') . '</b>');

        $contents = array('form' => tep_draw_form('categories', FILENAME_FUNWAYS, 'action=update_category&cPath=' . $cPath, 'post', 'enctype="multipart/form-data"') . tep_draw_hidden_field('categories_id', $cInfo->categories_id));
        ...
        $contents[] = array('align' => 'center', 'text' => '<br>' . htmlBase::newElement('button')->usePreset('save')->setType('submit')->draw() . ' ' . htmlBase::newElement('button')->usePreset('cancel')->setHref(tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id))->draw());
        break;
      case 'delete_category':
        $heading[] = array('text' => '<b>' . sysLanguage::get('TEXT_INFO_HEADING_DELETE_CATEGORY') . '</b>');

        $contents = array('form' => tep_draw_form('categories', FILENAME_FUNWAYS, 'action=delete_category_confirm&cPath=' . $cPath) . tep_draw_hidden_field('categories_id', $cInfo->categories_id));
        ...
        $contents[] = array('align' => 'center', 'text' => '<br>' . htmlBase::newElement('button')->usePreset('delete')->setType('submit')->draw() . ' ' . htmlBase::newElement('button')->usePreset('cancel')->setHref(tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id))->draw());
        break;
      case 'move_category':
        $heading[] = array('text' => '<b>' . sysLanguage::get('TEXT_INFO_HEADING_MOVE_CATEGORY') . '</b>');

        $contents = array('form' => tep_draw_form('categories', FILENAME_FUNWAYS, 'action=move_category_confirm&cPath=' . $cPath) . tep_draw_hidden_field('categories_id', $cInfo->categories_id));
        ...
        $contents[] = array('align' => 'center', 'text' => '<br>' . htmlBase::newElement('button')->usePreset('move')->setType('submit')->draw() . ' ' . htmlBase::newElement('button')->usePreset('cancel')->setHref(tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id))->draw());
        break;
      case 'copy_to':
        $heading[] = array('text' => '<b>' . sysLanguage::get('TEXT_INFO_HEADING_COPY_TO') . '</b>');

        $contents = array('form' => tep_draw_form('copy_to', FILENAME_FUNWAYS, 'action=copy_to_confirm&cPath=' . $cPath) . tep_draw_hidden_field('products_id', $pInfo->products_id));
        ...
        $contents[] = array('align' => 'center', 'text' => '<br>' . htmlBase::newElement('button')->usePreset('copy')->setType('submit')->draw() . ' ' . htmlBase::newElement('button')->usePreset('cancel')->setHref(tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&pID=' . $pInfo->products_id))->draw());
        break;
      ...
      while ($funways = tep_db_fetch_array($funways_query)) {
        ...
        $contents = array('form' => tep_draw_form('newproduct', FILENAME_FUNWAYS, 'action=insert_product&cPath=' . $cPath, 'post', 'enctype="multipart/form-data"').tep_draw_hidden_field('categories_id', $cPath_array[sizeof($cPath_array)-1]));
      ...
      while ($funways = tep_db_fetch_array($funways_query)) {
        ...
        $contents = array('form' => tep_draw_form('products', FILENAME_FUNWAYS, 'action=update_product&cPath=' . $cPath, 'post', 'enctype="multipart/form-data"') . tep_draw_hidden_field('products_id', $pInfo->products_id).tep_draw_hidden_field('categories_id', $cPath));
        $contents[] = array('text' => sysLanguage::get('TEXT_EDIT_INTRO'));
      ...
      case 'delete_product':
        $heading[] = array('text' => '<b>' . sysLanguage::get('TEXT_INFO_HEADING_DELETE_PRODUCT') . '</b>');
			...
			$contents = array('form' => tep_draw_form('products', FILENAME_FUNWAYS, 'action=delete_product_confirm&cPath=' . $cPath) . tep_draw_hidden_field('products_id', $pInfo->products_id));
			...
			$contents[] = array('align' => 'center', 'text' => '<br>' . htmlBase::newElement('button')->usePreset('delete')->setType('submit')->draw() . ' ' . htmlBase::newElement('button')->usePreset('cancel')->setHref(tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&pID=' . $pInfo->products_id))->draw());
        break;
      ...
      default:
        if ($rows > 0) {
          if (isset($cInfo) && is_object($cInfo)) { // category info box contents
            ...
            $contents[] = array('align' => 'center', 'text' => htmlBase::newElement('button')->usePreset('edit')->setHref(tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id . '&action=edit_category'))->draw() . ' ' . htmlBase::newElement('button')->usePreset('delete')->setHref(tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id . '&action=delete_category'))->draw() . ' ' . htmlBase::newElement('button')->usePreset('move')->setHref(tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id . '&action=move_category'))->draw());
            ...
            $contents[] = array('align' => 'center', 'text' => htmlBase::newElement('button')->usePreset('edit')->setHref(tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&pID=' . $pInfo->products_id . '&action=edit_product'))->draw() . ' ' . htmlBase::newElement('button')->usePreset('delete')->setHref(tep_href_link(FILENAME_FUNWAYS, 'cPath=' . $cPath . '&pID=' . $pInfo->products_id . '&action=delete_product'))->draw());
