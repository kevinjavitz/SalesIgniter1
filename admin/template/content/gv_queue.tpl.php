...
<?php
  ...
  switch ($_GET['action']) {
    case 'release':
      ...
      $contents[] = array('align' => 'center', 'text' => htmlBase::newElement('button')->setText(sysLanguage::get('IMAGE_CONFIRM'))->setHref(tep_href_link('gv_queue.php','action=confirmrelease&gid='.$gInfo->unique_id,'NONSSL'))->draw() . ' ' . htmlBase::newElement('button')->usePreset('cancel')->setHref(tep_href_link('gv_queue.php','action=cancel&gid=' . $gInfo->unique_id,'NONSSL'))->draw() . '</a>');
      break;
    default:
      $heading[] = array('text' => '[' . $gInfo->unique_id . '] ' . tep_datetime_short($gInfo->date_created) . ' ' . $currencies->format($gInfo->amount));

      $contents[] = array('align' => 'center','text' => htmlBase::newElement('button')->setText(sysLanguage::get('IMAGE_RELEASE'))->setHref(tep_href_link('gv_queue.php','action=release&gid=' . $gInfo->unique_id,'NONSSL'))->draw());
      break;
   }
