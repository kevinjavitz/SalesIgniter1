...
// Cache the categories box
  function tep_cache_categories_box($auto_expire = false, $refresh = false) {
    global $cPath, $tree, $cPath_array, $categories_string;

    if (($refresh == true) || !read_cache($cache_output, 'categories_box-' . Session::get('language') . '.cache' . $cPath, $auto_expire)) {
      ob_start();
      include(DIR_WS_BOXES . 'categories.php');
      $cache_output = ob_get_contents();
      ob_end_clean();
      write_cache($cache_output, 'categories_box-' . Session::get('language') . '.cache' . $cPath);
    }
...
// Cache the manufacturers box
  function tep_cache_manufacturers_box($auto_expire = false, $refresh = false) {
    $manufacturers_id = '';
    if (isset($_GET['manufactuers_id']) && is_numeric($_GET['manufacturers_id'])) {
    ...
    if (isset($_GET['products_id']) && is_numeric($_GET['products_id'])) {
      if (($refresh == true) || !read_cache($cache_output, 'also_purchased-' . Session::get('language') . '.cache' . $_GET['products_id'], $auto_expire)) {
        ob_start();
        include(DIR_WS_MODULES . FILENAME_ALSO_PURCHASED_PRODUCTS);
        $cache_output = ob_get_contents();
        ob_end_clean();
        write_cache($cache_output, 'also_purchased-' . Session::get('language') . '.cache' . $_GET['products_id']);
      }
    }
