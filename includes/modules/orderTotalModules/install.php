<?php
	class OrderTotalInstaller {
		
		public function __construct($moduleCode, $extName = null){
			$this->moduleDir = sysConfig::getDirFsCatalog();
			if (is_null($extName) === false){
				$this->moduleDir .= 'extensions/' . $extName . '/orderTotalModules/' . $moduleCode . '/';
			}else{
				$this->moduleDir .= 'includes/modules/orderTotalModules/' . $moduleCode . '/';
			}
			...
			$className = 'OrderTotal' . ucfirst($moduleCode);
