<?php
class OrderTotalGiftvoucher extends OrderTotalModule {
	...
	public function __construct(){
		...
		if ($this->isInstalled() === true){
			...
			$this->checkbox = htmlBase::newElement('checkbox')->setId('voucherPayment')->setName('c' . $this->getCode());
		}
	}
	...
	public function update_credit_account($cartProduct) {
		...
		if (ereg('^GIFT', addslashes($cartProduct->getModel()))) {
			...
			if ($this->credit_tax=='True') $gv_order_amount = $gv_order_amount * (100 + $cartProduct->getTaxRate()) / 100;
			...
			if (MODULE_ORDER_TOTAL_GV_QUEUE == 'false'){
				...
				if ($customer_gv === true){
					$gvQuery = Doctrine_Core::getTable('CouponGvCustomer')->findOneByCustomerId($userAccount->getCustomerId());
				} else {
	...
	public function credit_selection() {
		...
		if ($Qcoupon){
			...
			$image_submit = htmlBase::newElement('button')->setType('submit')->attr('onclick', 'submitFunction()')->setName('submit_redeem')->setText(sysLanguage::get('IMAGE_REDEEM_VOUCHER'))->draw();
		...
		if (array_key_exists('gv_redeem_code', $_POST)){
			...
			if ($Qcoupon){
				...
				if ($Qcheck['total'] > 0 && $coupon['coupon_type'] == 'G'){
					tep_redirect(itw_app_link('payment_error='.$this->code.'&error=' . urlencode(sysLanguage::get('ERROR_NO_INVALID_REDEEM_GV')), 'checkout', 'default', 'SSL'));
				}
			...
			if ($coupon['coupon_type'] == 'G'){
				...
				tep_redirect(itw_app_link('payment_error='.$this->code.'&error=' . urlencode(sysLanguage::get('ERROR_REDEEMED_AMOUNT'). $currencies->format($coupon_amount)), 'checkout', 'default', 'SSL'));
			}
		}

		if (array_key_exists('submit_redeem_x', $_POST) && !array_key_exists('gv_redeem_code', $_POST)){
			tep_redirect(itw_app_link('payment_error='.$this->code.'&error=' . urlencode(sysLanguage::get('ERROR_NO_REDEEM_CODE')), 'checkout', 'default', 'SSL'));
		}
	}
