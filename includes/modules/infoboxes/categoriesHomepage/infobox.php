...
class infoBoxCategoriesHomepage extends InfoBoxAbstract {

	public function __construct(){
		global $App;
		$this->init('categoriesHomepage');
	...
	public function show(){
		...
		if ($Result){
			...
			foreach($Result as $idx => $cInfo){
				...
				$liElement = htmlBase::newElement('a');
				/*if (isset($cPath_array) && $cPath_array[0] == $categoryId){
					$headerEl->addClass('currentCategory');
				}*/
				$liElement->html($categoryName);
				$liElement->setHref(itw_app_link('cPath=' . $categoryId, 'index', 'default'));


				$headerEl = htmlBase::newElement('list');
				$liElement1 = htmlBase::newElement('li')
					->html($liElement);
				$headerEl->addItemObj($liElement1);
				...
				if ($currentChildren->count() > 0){
					...
					->html(sysLanguage::get('INFOBOX_CATEGORIES_VIEW_PRODUCTS'))
		...
		$this->setBoxContent($menuContainer->draw() );
