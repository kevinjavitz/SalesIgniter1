...
sysLanguage::loadDefinitions(sysConfig::getDirFsCatalog() . 'includes/languages_phar/' . Session::get('language') . '/osc/article_header_tags.xml');
...
// ARTICLES.PHP
    ...
    $the_topic_query = tep_db_query("select td.topics_name from " . TABLE_TOPICS . " t, " . TABLE_TOPICS_DESCRIPTION . " td where t.topics_id = '" . (int)$current_topic_id . "' and td.topics_id = '" . (int)$current_topic_id . "' and td.language_id = '" . (int)Session::get('languages_id') . "'");
    ...
    if (HTTA_ARTICLES_ON=='1') {
      ...
      if (tep_not_null($the_topic['topics_name'])) {
        ...
        if (tep_not_null($the_authors['authors_name'])) {
          $the_title .= sysLanguage::get('TEXT_BY') . $the_authors['authors_name'];
        }
      }
...
// ARTICLE_INFO.PHP
  case ( strstr($_SERVER['PHP_SELF'],'article_info.php') or strstr($PHP_SELF,'article_info.php') ):
    ...
    if (empty($the_article_info['articles_head_keywords_tag'])) {
      $the_key_words= sysLanguage::get('NAVBAR_TITLE') . ', ' . HEAD_KEY_TAG_ALL;
    } else {
    ...
    if (empty($the_article_info['articles_head_title_tag'])) {
      $the_title= HEAD_TITLE_TAG_ALL . ' - ' . sysLanguage::get('NAVBAR_TITLE');
    } else {
  ...
  case ( strstr($_SERVER['PHP_SELF'],'articles_new.php') or strstr($PHP_SELF,'articles_new.php') ):
    ...
    if ( HEAD_KEY_TAG_ARTICLES_NEW=='' ) {
      ...
      if ( HTKA_ARTICLES_NEW_ON=='1' ) {
        $the_key_words= sysLanguage::get('NAVBAR_TITLE') . ', ' . $the_new_articles . ', ' . HEAD_KEY_TAG_ALL;
      } else {
        $the_key_words= sysLanguage::get('NAVBAR_TITLE') . ', ' . $the_new_articles;
      }
    } else {
  ...
  case ( strstr($_SERVER['PHP_SELF'],'article_reviews_info.php') or strstr($_SERVER['PHP_SELF'],'article_reviews.php') or strstr($PHP_SELF,'article_reviews_info.php') or strstr($PHP_SELF,'article_reviews.php') ):
    if ( HEAD_DESC_TAG_ARTICLE_REVIEWS_INFO=='' ) {
      if ( HTDA_ARTICLE_REVIEWS_INFO_ON=='1' ) {
        $the_desc= sysLanguage::get('NAVBAR_TITLE') . '. ' . tep_get_header_tag_articles_desc(isset($_GET['reviews_id'])) . ' ' . HEAD_DESC_TAG_ALL;
      } else {
        $the_desc= sysLanguage::get('NAVBAR_TITLE') . '. ' . tep_get_header_tag_articles_desc(isset($_GET['reviews_id']));
      }
    } else {
    ...
    if ( HEAD_KEY_TAG_ARTICLE_REVIEWS_INFO=='' ) {
      if ( HTKA_ARTICLE_REVIEWS_INFO_ON=='1' ) {
        $the_key_words= sysLanguage::get('NAVBAR_TITLE') . ', ' . tep_get_header_tag_articles_keywords(isset($_GET['reviews_id'])) . ', ' . HEAD_KEY_TAG_ALL;
      } else {
        $the_key_words= sysLanguage::get('NAVBAR_TITLE') . ', ' . tep_get_header_tag_articles_keywords(isset($_GET['reviews_id']));
      }
    } else {
    ...
    if ( HEAD_TITLE_TAG_ARTICLE_REVIEWS_INFO=='' ) {
      if ( HTTA_ARTICLE_REVIEWS_INFO_ON=='1' ) {
        $the_title= HEAD_TITLE_TAG_ALL . ' - ' . sysLanguage::get('HEADING_TITLE') . tep_get_header_tag_articles_title(isset($_GET['reviews_id']));
      } else {
