...
function check_form(form_name) {
  if (submitted == true) {
    alert("<?php echo sysLanguage::get('JS_ERROR_SUBMITTED'); ?>");
    return false;
  }

  error = false;
  form = form_name;
  error_message = "<?php echo sysLanguage::get('JS_ERROR'); ?>";
...
<?php if (ACCOUNT_STATE == 'true') echo '  check_input("state", ' . ENTRY_STATE_MIN_LENGTH . ', "' . ENTRY_STATE_ERROR . '");' . "\n"; ?>
  ...
  check_select("country", "", "<?php echo sysLanguage::get('ENTRY_COUNTRY_ERROR'); ?>");

  check_input("telephone", <?php echo ENTRY_TELEPHONE_MIN_LENGTH; ?>, "<?php echo ENTRY_TELEPHONE_NUMBER_ERROR; ?>");

  check_password("password", "confirmation", <?php echo ENTRY_PASSWORD_MIN_LENGTH; ?>, "<?php echo ENTRY_PASSWORD_ERROR; ?>", "<?php echo sysLanguage::get('ENTRY_PASSWORD_ERROR_NOT_MATCHING'); ?>");
