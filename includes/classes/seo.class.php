	...
	function init(){
		...
		switch(true){
			case ( !$this->attributes['IS_DEFINED'] ):
				$this->eval_defaults();
				$this->DB = new SEO_DataBase(sysConfig::get('DB_SERVER'), sysConfig::get('DB_SERVER_USERNAME'), sysConfig::get('DB_DATABASE'), sysConfig::get('DB_SERVER_PASSWORD'));
 ...
 */	
	function SEO_URL($languages_id){
		...
		$this->attributes = array('PHP_VERSION' => PHP_VERSION,
								  ...
								  'SID' => $SID,
								  'SEO_ENABLED' => sysConfig::exists('SEO_ENABLED') ? sysConfig::get('SEO_ENABLED') : 'false',
								  'SEO_ADD_CPATH_TO_PRODUCT_URLS' => sysConfig::exists('SEO_ADD_CPATH_TO_PRODUCT_URLS') ? sysConfig::get('SEO_ADD_CPATH_TO_PRODUCT_URLS') : 'false',
								  'SEO_ADD_CAT_PARENT' => sysConfig::exists('SEO_ADD_CAT_PARENT') ? sysConfig::get('SEO_ADD_CAT_PARENT') : 'true',
								  'SEO_URLS_USE_W3C_VALID' => sysConfig::exists('SEO_URLS_USE_W3C_VALID') ? sysConfig::get('SEO_URLS_USE_W3C_VALID') : 'true',
								  'USE_SEO_CACHE_GLOBAL' => sysConfig::exists('USE_SEO_CACHE_GLOBAL') ? sysConfig::get('USE_SEO_CACHE_GLOBAL') : 'false',
								  'USE_SEO_CACHE_PRODUCTS' => sysConfig::exists('USE_SEO_CACHE_PRODUCTS') ? sysConfig::get('USE_SEO_CACHE_PRODUCTS') : 'false',
								  'USE_SEO_CACHE_CATEGORIES' => sysConfig::exists('USE_SEO_CACHE_CATEGORIES') ? sysConfig::get('USE_SEO_CACHE_CATEGORIES') : 'false',
								  'USE_SEO_CACHE_MANUFACTURERS' => sysConfig::exists('USE_SEO_CACHE_MANUFACTURERS') ? sysConfig::get('USE_SEO_CACHE_MANUFACTURERS') : 'false',
								  'USE_SEO_CACHE_ARTICLES' => sysConfig::exists('USE_SEO_CACHE_ARTICLES') ? sysConfig::get('USE_SEO_CACHE_ARTICLES') : 'false',
								  'USE_SEO_CACHE_TOPICS' => sysConfig::exists('USE_SEO_CACHE_TOPICS') ? sysConfig::get('USE_SEO_CACHE_TOPICS') : 'false',
								  'USE_SEO_CACHE_INFO_PAGES' => sysConfig::exists('USE_SEO_CACHE_INFO_PAGES') ? sysConfig::get('USE_SEO_CACHE_INFO_PAGES') : 'false',
								  'USE_SEO_REDIRECT' => sysConfig::exists('USE_SEO_REDIRECT') ? sysConfig::get('USE_SEO_REDIRECT') : 'false',
								  'SEO_REWRITE_TYPE' => sysConfig::exists('SEO_REWRITE_TYPE') ? sysConfig::get('SEO_REWRITE_TYPE') : 'false',
								  'SEO_URLS_FILTER_SHORT_WORDS' => sysConfig::exists('SEO_URLS_FILTER_SHORT_WORDS') ? sysConfig::get('SEO_URLS_FILTER_SHORT_WORDS') : 'false',
								  'SEO_CHAR_CONVERT_SET' => sysConfig::exists('SEO_CHAR_CONVERT_SET') ? $this->expand(sysConfig::get('SEO_CHAR_CONVERT_SET')) : 'false',
								  'SEO_REMOVE_ALL_SPEC_CHARS' => sysConfig::exists('SEO_REMOVE_ALL_SPEC_CHARS') ? sysConfig::get('SEO_REMOVE_ALL_SPEC_CHARS') : 'false',
								  'SEO_PAGES' => $seo_pages,
								  'SEO_INSTALLER' => $this->installer->attributes
								  );		
		
		$this->base_url = sysConfig::get('HTTP_SERVER') . sysConfig::get('DIR_WS_HTTP_CATALOG');
		$this->base_url_ssl = sysConfig::get('HTTPS_SERVER') . sysConfig::get('DIR_WS_HTTPS_CATALOG');
		...
		if ($this->attributes['SEO_ENABLED'] == 'true' && $this->attributes['USE_SEO_CACHE_GLOBAL'] == 'true'){
		...
		EventManager::notify('SeoUrlsInit', &$this);
 ...
 */	
	function href_link($page = '', $parameters = '', $connection = 'NONSSL', $add_session_id = true){
		...
		switch($this->attributes['SEO_URLS_USE_W3C_VALID']){
			case ('true'):
				if (!isset($_SESSION['customer_id']) && sysConfig::exists('ENABLE_PAGE_CACHE') && sysConfig::get('ENABLE_PAGE_CACHE') == 'true' && class_exists('page_cache')){
					return $link;
				} else {
 ...
 */	
  function stock_href_link($page = '', $parameters = '', $connection = 'NONSSL', $add_session_id = true, $search_engine_safe = true) {
    ...
    $connection = (!empty($connection) ? $connection : 'NONSSL');
    ...
    if ( ($add_session_id == true) && ($session_started == true) && (sysConfig::get('SESSION_FORCE_COOKIE_USE') == 'False') ) {
      if ($this->not_null($SID)) {
        $_sid = $SID;
      } elseif ( ( ($request_type == 'NONSSL') && ($connection == 'SSL') && (sysConfig::get('ENABLE_SSL') == true) ) || ( ($request_type == 'SSL') && ($connection == 'NONSSL') ) ) {
        if (sysConfig::get('HTTP_COOKIE_DOMAIN') != sysConfig::get('HTTPS_COOKIE_DOMAIN')) {
    ...
    switch(true){
		case (!isset($_SESSION['customer_id']) && sysConfig::exists('ENABLE_PAGE_CACHE') && sysConfig::get('ENABLE_PAGE_CACHE') == 'true' && class_exists('page_cache')):
			$page_cache = true;
 ...
 */	
	function add_sid( $link, $add_session_id, $connection, $separator ){
		...
		if ( ($add_session_id) && ($this->attributes['SESSION_STARTED']) && (sysConfig::get('SESSION_FORCE_COOKIE_USE') == 'False') ) {
		...
		switch(true){
			case (!isset($_SESSION['customer_id']) && sysConfig::exists('ENABLE_PAGE_CACHE') && sysConfig::get('ENABLE_PAGE_CACHE') == 'true' && class_exists('page_cache')):
				$return = $link . $separator . '<osCsid>';
				break;
			case ($this->not_null((isset($_sid) ? $_sid : null))):
				$return = $link . $separator . $_sid;
				break;
			default:
				$return = $link;
			...
			switch ($p2[0]){ 
				...
				case 'pID':
					switch(true){
						...
						$url = $this->make_url($page, $this->get_product_name($p2[1]), $p2[0], str_replace('?','',$p2[1]), '.html', $separator);
						break;
					default:
						$container[$p2[0]] = $p2[1];
						break;
 ...
 */	
	function make_url($page, $string, $anchor_type, $id, $extension = '.html', &$separator){
		...
		switch ( $this->attributes['SEO_REWRITE_TYPE'] ){
			case 'Rewrite':
				//mail('adm@localhost', 'debug make_url', '$string='.$string. '\n $this->reg_anchors[$anchor_type]='.$this->reg_anchors[$anchor_type] .'\n $id='.$id . '\n $extension='.$extension);
	...
	function get_cache($name = 'GLOBAL', $local_memory = false){
		...
		if ( $num_rows ){ 
			...
			while($cache = $this->DB->FetchArray($this->cache_query)){
				...
				if ( isset($this->keep_in_memory) || $local_memory ) {
 ...
 */	
	function need_redirect(){		
		...
		switch(true){
			case (isset($_GET['rType']) && $_GET['rType'] == 'ajax'):
				$this->need_redirect = false;
				break;
 ...
 */	
	function do_redirect(){
		...
		switch(true){
			case (sysConfig::exists('USE_SEO_REDIRECT_DEBUG') && sysConfig::get('USE_SEO_REDIRECT_DEBUG') == 'true'):
