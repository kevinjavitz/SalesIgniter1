...
function Calendar(element, options, eventSources) {
	...
	function setSize(dateChanged) { // todo: dateChanged?
		ignoreWindowResize++;
        //alert(content.width() + ' '+suggestedViewHeight);
...
function BasicWeekView(element, calendar) {
	...
	function render(date, delta) {
		if (delta) {
			addDays(date, delta *  opt('weekDayNumber'));
		}
		var start = addDays(cloneDate(date), -((date.getDay() - opt('firstDay') +  opt('weekDayNumber')) %  opt('weekDayNumber')));
		var end = addDays(cloneDate(start),  opt('weekDayNumber'));
		...
		t.visStart = visStart;
		t.visEnd = visEnd;
		renderBasic(1, weekends ?  opt('weekDayNumber') : 5, false);
	}
...
function BasicEventRenderer() {
	...
	function compileSegs(events) {
		...
		for (i=0; i<rowCnt; i++) {
			...
			addDays(d1,  opt('weekDayNumber'));
			addDays(d2,  opt('weekDayNumber'));
		}
		return segs;
	}
...
function AgendaView(element, calendar, viewName) {
	...
	function renderAgenda(c) {
		...
		if (!head) { // first time rendering, build from scratch
			...
			// head
			s = "<div class='fc-agenda-head ui-widget-header' style='position:relative;z-index:4'>" +
				"<table style='width:100%'>" +
				"<tr class='fc-first" + (opt('allDaySlot') ? '' : ' fc-last') + "'>" +
				"<th style='width:40px;' class='fc-leftmost " +
					"'>&nbsp;</th>";
			for (i=0; i<colCnt; i++) {
				s += "<th style='width:40px;' class='fc-" +
					dayIDs[d.getDay()] + ' ' + // needs to be first
			...
			for (i=0; d < maxd; i++) {
				...
				s += "<tr class='" +
					...
					"'><th style='width:40px;' class='fc-axis fc-leftmost " + tm + "-state-default'>" +
					((!slotNormal || !minutes) ? formatDate(d, opt('axisFormat')) : formatDate(d, opt('axisFormat'))) +
					"</th><td style='width:40px;' class='fc-slot" + i + ' ' +
	...
	function daySegHTML(segs) { // also sets seg.left and seg.outerWidth
		...
		for (i=0; i<segCnt; i++) {
			...
			if (rtl) {
				...
				//cols[0] = (seg.end - t.visStart)/(1000*60*60*24)-1;
				cols[0] = dayOfWeekCol(seg.end.getDay()-1);
				//cols[1] = (seg.start - t.visStart)/(1000*60*60*24);
                ...
                cols[1] = dayOfWeekCol(parseInt((seg.end.getTime() - t.visStart.getTime())/(1000*60*60*24)-1));
                //alert(cols[1]);
				cols[0] = dayOfWeekCol(parseInt((seg.start.getTime() - t.visStart.getTime())/(1000*60*60*24)));
                //alert(cols[0]);
				//cols[0] = dayOfWeekCol(seg.start.getDate());
				//cols[1] = dayOfWeekCol(seg.end.getDate()-1);
