...
if (isset($_GET['type']) && $_GET['type'] == 'addressBook'){
	...
	if ($_POST['currentPage'] == 'addresses') {
		...
		$accountValidation = array(
			'entry_firstname'      => $_POST['billing']['entry_firstname'],
			'entry_lastname'       => $_POST['billing']['entry_lastname'],
			'entry_street_address' => $_POST['billing']['entry_street_address'],
			'entry_postcode'       => $_POST['billing']['entry_postcode'],
			'entry_city'           => $_POST['billing']['entry_city'],
			'entry_country_id'     => $_POST['billing']['entry_country'],
		);
		...
		if (isset($_POST['billing']['entry_suburb'])) $accountValidation['entry_suburb'] = $_POST['billing']['entry_suburb'];
		if (isset($_POST['billing']['entry_state'])) $accountValidation['entry_state'] = $_POST['billing']['entry_state'];
		if (isset($_POST['billing']['entry_gender'])) $accountValidation['entry_gender'] = $_POST['billing']['entry_gender'];
		if (isset($_POST['billing']['entry_date_of_birth'])) $accountValidation['entry_dob'] = $_POST['billing']['entry_date_of_birth'];
		if (isset($_POST['billing']['entry_fiscal_code'])) $accountValidation['entry_cif'] = $_POST['billing']['entry_fiscal_code'];
		if (isset($_POST['billing']['entry_vat_number'])) $accountValidation['entry_vat'] = $_POST['billing']['entry_vat_number'];
		if (isset($_POST['billing']['entry_city_birth'])) $accountValidation['entry_city_birth'] = $_POST['billing']['entry_city_birth'];
		if (isset($_POST['billing']['entry_company'])) $accountValidation['entry_company'] = $_POST['billing']['entry_company'];
		if (isset($_POST['billing']['entry_telephone'])) $accountValidation['telephone'] = $_POST['billing']['entry_telephone'];
		if (isset($_POST['billing']['entry_fax'])) $accountValidation['fax'] = $_POST['billing']['entry_fax'];
		if ($userAccount->isLoggedIn() === false){
			if (isset($_POST['billing']['entry_email_address'])) $accountValidation['email_address'] = $_POST['billing']['entry_email_address'];
		}else{
		...
		$billingAddressArray = array(
			'entry_gender' => (isset($_POST['billing']['entry_gender']) ? $_POST['billing']['entry_gender'] : 'm'),
			'entry_dob' => (isset($_POST['billing']['entry_date_of_birth']) ? $_POST['billing']['entry_date_of_birth'] : ''),
			'entry_company' => (isset($_POST['billing']['entry_company'])?$_POST['billing']['entry_company']:''),
			'entry_firstname' => $_POST['billing']['entry_firstname'],
			'entry_lastname' => $_POST['billing']['entry_lastname'],
			'entry_street_address' => $_POST['billing']['entry_street_address'],
			'entry_suburb' => (isset($_POST['billing']['entry_suburb']) ? $_POST['billing']['entry_suburb'] : ''),
			'entry_postcode' => $_POST['billing']['entry_postcode'],
			'entry_city' => $_POST['billing']['entry_city'],
			'entry_cif' => (isset($_POST['billing']['entry_fiscal_code'])?$_POST['billing']['entry_fiscal_code']:''),
			'entry_vat' => (isset($_POST['billing']['entry_vat_number'])?$_POST['billing']['entry_vat_number']:''),
			'entry_city_birth' => (isset($_POST['billing']['entry_city_birth'])?$_POST['billing']['entry_city_birth']:''),
			'entry_state' => (isset($_POST['billing']['entry_state'])?$_POST['billing']['entry_state']:'none'),
			'entry_country_id' => $_POST['billing']['entry_country'],
			'entry_zone_id' => (isset($_POST['billing']['entry_state'])?$_POST['billing']['entry_state']:'0')
		);
		$userAccount->plugins['addressBook']->addAddressEntry('billing', $billingAddressArray);
		$userAccount->setFirstName($_POST['billing']['entry_firstname']);
		$userAccount->setLastName($_POST['billing']['entry_lastname']);
		...
		if (!empty($_POST['billing']['entry_telephone'])) {
			$onePageCheckout->onePage['info']['telephone'] = $_POST['billing']['entry_telephone'];
			$userAccount->setTelephoneNumber($onePageCheckout->onePage['info']['telephone']);
		}

		if (!empty($_POST['billing']['entry_email_address'])) {
			$onePageCheckout->onePage['info']['email_address'] = $_POST['billing']['entry_email_address'];
			$userAccount->setEmailAddress($onePageCheckout->onePage['info']['email_address']);
		}

		if (!empty($_POST['billing']['entry_date_of_birth'])) {
			$onePageCheckout->onePage['info']['dob'] = $_POST['billing']['entry_date_of_birth'];
		...
		if ($error == false) {
			...
			ob_start();
			require(sysConfig::getDirFsCatalog() . 'applications/mobile/pagesTabs/checkout/shipping_payment.php');
			$pageHtml = ob_get_contents();
			ob_end_clean();
		...
		if ($error === false && $messageStack->size('pageStack') == 0) {
			...
			if ($remotePayment === true) {
				...
				if ($messageStack->size('pageStack') == 0){
					ob_start();
					require(sysConfig::getDirFsCatalog() . 'applications/mobile/pagesTabs/checkout/success.php');
					$pageHtml = ob_get_contents();
					ob_end_clean();
				}
			}
...
if ($messageStack->size('pageStack') > 0) {
	$error = true;
	$pageHtml = '<div class="ui-bar ui-bar-e"><h3>' .
