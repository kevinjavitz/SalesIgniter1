...
class Securimage {
	...
	*/
	...
	function validate()
	{
		...
		if (Session::exists('securimage_code_value')) {
		...
		if ($code != '') {
			if ($code == $code_entered) {
			  ...
			  Session::remove('securimage_code_value');
			  Session::remove('securimage_code_ctime');
	...
	function saveCodeToDatabase()
	{
		...
		if ($this->use_sqlite_db && $this->sqlite_handle !== false) {
			$ip = $this->getIPHash();
			$time = time();
			$code = Session::get('securimage_code_value'); // hash code for security - if cookies are disabled the session still exists at this point
